...
{
  public:
    control_point(const glm::vec3 &position = {0.f, 0.f, 0.f}, const glm::vec3 &color = {.1f, 1.f, 1.f}, float radius = 0.05);

    // for creating centroid (create base points first)
    // takes CoonsSurface::basePoints_
    control_point(const std::vector<s_ptr<control_point>> &base_points, const glm::vec3 &color, float radius);

    // getter
    s_ptr<draggable_component> get_dragComp_sp() { return drag_comp_sp_; }
